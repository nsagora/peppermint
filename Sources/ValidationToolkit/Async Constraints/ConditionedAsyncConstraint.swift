...
public struct ConditionedAsyncConstraint<T>: AsyncConstraint {

    public typealias InputType = T

    private var predicate: AnyAsyncPredicate<InputType>
    private var errorBuilder: (InputType) -> Error
    private var hasConditions: Bool { conditions.count > 0 }
     ...
     */
    public init<P: AsyncPredicate>(predicate: P, error: @escaping (InputType) -> Error)
    where P.InputType == InputType {
     ...
     */
    public mutating func add<C: AsyncConstraint>(condition: C) where C.InputType == InputType {
    ...
    public func evaluate(with input: InputType, queue: DispatchQueue,completionHandler: @escaping (_ result: Result) -> Void) {
        ...
        let predicateSet = CompoundAsyncConstraint(allOf: conditions)
        predicateSet.evaluate(with: input, queue: queue) { result in

            if result.isSuccessful {
                return self.continueEvaluate(
                    with: input, queue: queue, completionHandler: completionHandler)
            }
